...
class DebridgeService:
    ...
    @Tool(
        {
            "description": "Get details for the chains supported by the deBridge Liquidity Network",
        }
    )
    async def get_supported_chains(self):
        """Get details for the chains supported by the deBridge Liquidity Network"""
        return await self._fetch("/supported-chains-info", "get supported chains")

    @Tool(
        {
            "description": "Get the token list supported by the deBridge Liquidity Network",
    ...
    @Tool(
        {
            "description": "Get the order IDs from the hash of the creation transaction",
    ...
    @Tool(
        {
            "description": "Generate a transaction that cancels external call in the given order",
    ...
    @Tool(
        {
            "description": "Generate the data for a transaction to place a cross-chain DLN order",
    ...
    async def create_order_transaction(self, parameters: dict):
        ...
        optional_params = {
            "dstChainTokenOutAmount": parameters.get("dstChainTokenOutAmount", "auto"),
            "additionalTakerRewardBps": parameters.get("additionalTakerRewardBps"),
            "srcIntermediaryTokenAddress": parameters.get("srcIntermediaryTokenAddress"),
            "dstIntermediaryTokenAddress": parameters.get("dstIntermediaryTokenAddress"),
            "dstIntermediaryTokenSpenderAddress": parameters.get("dstIntermediaryTokenSpenderAddress"),
            "intermediaryTokenUSDPrice": parameters.get("intermediaryTokenUSDPrice"),
            "dstChainTokenOutRecipient": parameters.get("dstChainTokenOutRecipient"),
            "senderAddress": parameters.get("senderAddress"),
            "srcChainOrderAuthorityAddress": parameters.get("srcChainOrderAuthorityAddress"),
            "srcAllowedCancelBeneficiary": parameters.get("srcAllowedCancelBeneficiary"),
            "referralCode": parameters.get("referralCode", 31494),
            "affiliateFeePercent": parameters.get("affiliateFeePercent", 0),
            "affiliateFeeRecipient": parameters.get("affiliateFeeRecipient"),
            "srcChainTokenInSenderPermit": parameters.get("srcChainTokenInSenderPermit"),
            "dstChainOrderAuthorityAddress": parameters.get("dstChainOrderAuthorityAddress"),
            "enableEstimate": parameters.get("enableEstimate"),
            "allowedTaker": parameters.get("allowedTaker"),
            "dlnHook": parameters.get("dlnHook"),
            "prependOperatingExpenses": parameters.get("prependOperatingExpenses", False),
            "metadata": parameters.get("metadata"),
            "ptp": parameters.get("ptp"),
            "skipSolanaRecipientValidation": parameters.get("skipSolanaRecipientValidation", False)
        }
