...
export const IrysPaymentToken = z.enum([
    "ethereum",
    "matic",
    "bnb",
    "avalanche",
...
export class UploadFolderParameters extends createToolParameters(
    z.object({
        ...
        indexFile: z.string().optional().default("").describe("Optional index file (file the user will load when accessing the manifest)"),
        batchSize: z.number().optional().default(50).describe("Number of items to upload at once"),
        keepDeleted: z.boolean().optional().default(false).describe("Boolean determining whether to keep now deleted items from previous uploads"),
    }),
) { }
